...
<table>
<tr><td colspan="2"><a href="#1-core-papers-deep-dives">1. Core Papers (Deep Dives)</a></td></tr>
<tr><td colspan="2"><a href="#2-surveys-and-analyses">2. Surveys and Analyses</a></td></tr>
<tr><td colspan="2"><a href="#3-benchmarks-for-memory-dependent-manipulation">3. Benchmarks for Memory-Dependent Manipulation</a></td></tr>
<tr><td colspan="2"><a href="#4-event-and-keyframe-memory">4. Event and Keyframe Memory</a></td></tr>
<tr><td colspan="2"><a href="#5-dense-and-compressed-visual-history">5. Dense and Compressed Visual History</a></td></tr>
<tr><td colspan="2"><a href="#6-latent-recurrent-and-slot-memory">6. Latent, Recurrent and Slot Memory</a></td></tr>
<tr><td colspan="2"><a href="#7-dual-system-agentic-and-symbolic-memory">7. Dual-System, Agentic and Symbolic Memory</a></td></tr>
<tr><td colspan="2"><a href="#8-memory-inside-world-and-video-action-models">8. Memory inside World and Video Action Models</a></td></tr>
...
1. **EventVLA: Event-Driven Visual Evidence Memory for Long-Horizon Vision-Language-Action Policies.** arXiv 2026. [paper](https://arxiv.org/abs/2606.20092) [pdf](papers/pdf/EventVLA_2606.20092.pdf) [zh](papers/zh/EventVLA_2606.20092_zh.pdf) [report](reports/01_eventvla_en.md) [note](notes/EventVLA_2606.20092.md)
...
2. **TRACE: Trajectory-Routed Causal Memory for Delayed-Evidence Visuomotor Imitation.** arXiv 2026. [paper](https://arxiv.org/abs/2606.14551) [pdf](papers/pdf/TRACE_2606.14551.pdf) [zh](papers/zh/TRACE_2606.14551_zh.pdf) [report](reports/02_trace_en.md) [note](notes/TRACE_2606.14551.md)

    *Zihao Li, Ranpeng Qiu, Yincong Chen, Guoqiang Ren, Weiming Zhi*

3. **Robots that Collaborate: Sequential Asymmetric Imitation for Learning Coupled Robot Policies.** arXiv 2026. [paper](https://arxiv.org/abs/2606.16490) [pdf](papers/pdf/SAI_2606.16490.pdf) [zh](papers/zh/SAI_2606.16490_zh.pdf) [report](reports/03_sai_en.md) [note](notes/SAI_2606.16490.md)
...
Full analysis in [reports/04_trends_insights_en.md](reports/04_trends_insights_en.md) ([中文](reports/04_trends_insights_cn.md)). The ten insights in one line each:

1. The frontier moved from how much to store to when to write — EventVLA, KEMO, UniMem, WeaveLA, MemoryWAM and TFP converge on event-driven sparse writes from different architectures
2. Present but Not Remembered supplies the mechanism: history inside frozen VLAs is largely a redundant copy of the present; memory augmentation should inject information unique to the past
3. Raw frames versus latents depends on how many bits must be recovered — EventVLA's implicit-bank ablation (24.9 vs 75.2) and TRACE's 512-D slots (69.2) do not contradict each other
4. Addressing is the under-rated axis — TRACE's trajectory signatures give write time and read time a stable correspondence
5. Long context is less brittle than claimed, but spurious correlation is real — usability depends on whether supervision steers attention to task-relevant history
6. Reliability comes from disciplined state updates, not capacity — AGM's verify-then-advance, EventVLA's NMS + cooldown and TRACE's gated writes are one principle
7. Dual systems still lead on counting and procedural memory — RoboMME / RoboMemArena leaders (PonderPounce, HyMeS, BATON) keep state at a high level
8. The cost of memory is heading toward zero — NativeMEM one token per frame, TempoFit training-free, StreamPI zero parameters; EventVLA's threefold latency will be the first thing its successors optimise
...
```bash
python3 scripts/build_manifest.py        # arXiv metadata from scripts/curated_ids.tsv -> scripts/papers_manifest.json
python3 scripts/make_notes.py            # notes/
...
Pull requests are welcome: add a line to `scripts/curated_ids.tsv` (group, short key, arXiv id, venue/year), a one-line positioning to `scripts/oneliners_cn.json`, then run the generators above. Conventions are in [CONTRIBUTING.md](CONTRIBUTING.md).
